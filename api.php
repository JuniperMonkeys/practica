    ...
    switch ($action) {
        ...
        case 'get_workspace_details':
            ...
            if (!empty($event_ids)) {
                ...
                $stmt = $pdo->prepare("SELECT id, event_id, title, subtitle, is_flagged, is_done, link_url, item_date, start_time, end_date, end_time, is_divider, sort_order FROM event_items WHERE event_id IN ($in) ORDER BY sort_order ASC, item_date ASC, created_at ASC");
            ...
            $stmt = $pdo->prepare("SELECT id, name, notes, owner_id FROM workspaces WHERE id = ?");
            $stmt->execute([$workspace_id]);
