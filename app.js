const App = {
	...
	loginWithCAS: async () => {
		// SSO flow
		// In a live environment, this would redirect to https://cas.ucdavis.edu.
		// For development, this is just a sim path.
		const username = prompt("Enter a campus username to simulate CAS login:", "woangel");
		if (!username) return;

		const res = await App.api({ action: 'cas_sim', username: username });
		if (res.success) {
			App.state.user = res.user;
			App.loadWorkspaces();
			App.updateNav();
		} else {
			alert(res.error || "CAS Login failed");
		}
	},
		...
		app.innerHTML = `
			<div class="auth-container">
				...
				<div class="card auth-form">
					...
					<button class="btn btn-primary" style="background: #002855; color: white; border: 1px solid #1c355e; height: 50px; font-weight: bold; width: 100%; border-radius: 8px; margin-bottom: 20px;" onclick="App.loginWithCAS()">
						Test a CAS Login
					</button>

					<div class="auth-divider"><span>Local Login</span></div>
					...
					<button class="btn btn-secondary" style="width: 100%; margin-top: 10px;" onclick="App.handleLoginBtn()">Sign In</button>
					<p style="margin-top: 15px; text-align: center; font-size: 0.9rem;">
						<a href="#" onclick="App.renderRegister()" style="color: var(--color-sea-blue); text-decoration: none;">Create a local account</a>
					</p>
	...
	<div class="event-detail-header">
		<input type="text" class="event-title-input" value="${event.title ? event.title.replace(/"/g, '&quot;') : ''}" onblur="App.updateEventField('title', this.value)">
			...
			<div class="event-action-bar">
				...
				<button onclick="App.deleteEvent(${event.id})" class="btn btn-secondary btn-sm delete-event-btn" style="color: #ef5350;"><i class="fa-solid fa-trash"></i> Delete Event</button>
